...
def hist_data(filepath: str) -> pd.DataFrame:
    '''
...
def _bizdays(year: int) -> pd.DataFrame:
    biz_days = {'date': jpbizday.year_bizdays(year)}
    bizdays = pd.DataFrame(biz_days).set_index('date')
    return bizdays


def hist_data_with_adj_close(input_filepath: str, code: str, year: str,
    ...
    ret = _bizdays(year).merge(adj_rate_for_current_stock, \
...
def main():
    args = parse_args()
    if args.dill_input:
        adj_rate_df = load_from_dill(args.dill_input)
    else:
        adj_rate_df = get_adj_rate((args.heigou_input, args.bunkatsu_input))
    # adj_rate_df = get_adj_rate(['heigou.html', \
    #                             'bunkatsu.html'])
    if args.dill_output:
        save_as_dill(adj_rate_df, args.dill_output)

    year = args.year
    # year = 2020
    codes = args.codes.split(',')
    # codes = ['9143', '2413']
    [hist_data_with_adj_close(args.input, code, year, adj_rate_df) \
